...
from src.agent import ActorCriticMLP, ActorCriticTransformer
...
def main():
    ...
    # --- Setup Environment ---
    # Use seq_length as stack_size for transformer, default 8 for MLP
    stack_size = args.seq_length if args.use_transformer else 8
    ...
    if args.use_transformer:
        agent = ActorCriticTransformer(envs, seq_length=args.seq_length).to(device)
        print(f"Using Transformer architecture (seq_length={args.seq_length})")
    else:
        agent = ActorCriticMLP(envs).to(device)
            ...
            for step in pbar:
                ...
                for i, item in enumerate(info):
                    if "final_info" in item and item["final_info"] is not None:
                        ...
                        writer.add_scalar("charts/drift_boosts", ep_info.get("drift_boosts", 0), global_step)
                        writer.add_scalar("charts/mushrooms_used", ep_info.get("mushrooms_used", 0), global_step)
                        writer.add_scalar("charts/mushrooms_wasted", ep_info.get("mushrooms_wasted", 0), global_step)
