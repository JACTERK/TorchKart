...
class MK64Env(gym.vector.VectorEnv):
    ...
    # 5 * 4 (float) + 5 * 4 (int) + 1 * 2 (short uint) + 2 * 4 (drift/boost int) = 50
    STATE_SIZE_BYTES = 50

    # The struct format string for parsing the 50 bytes.
    # '>' means big-endian (which N64 is)
    # f = 4-byte float
    # i = 4-byte signed int
    # H = 2-byte unsigned short
    # Fixed-Point 16.16 is read as 'i' (signed int)
    STATE_FORMAT = ">fffiiHiiffiii"
    ...
    # Original 10 + drift_active, drift_duration, mushroom_boost_active, angular_velocity
    NUM_OBS_FEATURES = 14
    ...
    def __init__(self, num_envs, host, port, frame_skip=4, stack_size=8, emulator_manager=None):
        ...
        # Derived feature trackers (per-env, persistent across steps)
        self.drift_counters = np.zeros(num_envs, dtype=np.float32)
        self.last_angles = np.zeros(num_envs, dtype=np.float32)

        # Reward shaping trackers (per-env)
        self.was_drifting = np.zeros(num_envs, dtype=np.bool_)

        # Per-episode strategic metric trackers
        self.episode_drift_boosts = np.zeros(num_envs, dtype=np.int32)
        self.episode_mushrooms_used = np.zeros(num_envs, dtype=np.int32)
        self.episode_mushrooms_wasted = np.zeros(num_envs, dtype=np.int32)
    ...
    def _parse_and_preprocess(self, state_bytes: bytes, env_index: int, old_wall_values: Tuple[int, int] = None) -> Tuple[np.ndarray, Dict[str, Any]]:
        """
        ...
        # Speed: Range 0 to 80 -> Map to 0.0 to 1.0
        ...
        # --- Drift State ---
        # 0=not drifting, 2=drifting on road, 4=drifting off-road
        drift_active = 1.0 if state["drift_state"] != 0 else 0.0

        # Drift duration counter: track how long we've been drifting (normalized by 180 frames = 3 sec)
        if drift_active > 0.5:
            self.drift_counters[env_index] += 1.0
        else:
            self.drift_counters[env_index] = 0.0
        drift_duration = min(self.drift_counters[env_index] / 180.0, 1.0)

        # --- Mushroom Boost Status ---
        # 0=inactive, 8192=active
        mushroom_boost_active = 1.0 if state["mushroom_boost_raw"] == 8192 else 0.0

        # --- Angular Velocity ---
        # Difference between current and previous angle, handling wraparound
        prev_angle = self.last_angles[env_index]
        angular_diff = angle_radians - prev_angle
        # Handle wraparound (if diff > pi, we wrapped around)
        if angular_diff > np.pi:
            angular_diff -= 2 * np.pi
        elif angular_diff < -np.pi:
            angular_diff += 2 * np.pi
        self.last_angles[env_index] = angle_radians
        # Normalize: max reasonable angular velocity ~0.5 rad/step
        angular_velocity = np.clip(angular_diff / 0.5, -1.0, 1.0)

        # --- Build Observation Vector (14 features) ---
        obs = np.array([
            norm_x_vel,
            norm_y_vel,
            norm_speed,
        ...
        processed_state_dict = {
            ...
            "mushroom_boost_raw": state["mushroom_boost_raw"],
            "angular_velocity": angular_diff,
        }

        return obs, processed_state_dict

    def _calculate_reward(self, env_index: int, old_state: Dict, new_state: Dict) -> Tuple[float, bool, bool]:
        """
        ...
        # Reward for speed (Between 0 and 80)
        speed = new_state["speed"]

        if speed > 80:
            reward += (speed / 80.0) * 0.5
        ...
        # Drift Boost Reward
        currently_drifting = new_state["drift_state"] != 0
        was_drifting = self.was_drifting[env_index]

        if was_drifting and not currently_drifting:
            # Drift just ended — check if we got a boost
            if new_state["mushroom_boost_raw"] == 8192:
                reward += 5.0  # Successful drift boost
                self.episode_drift_boosts[env_index] += 1
            else:
                reward -= 0.5  # Released drift too early or hit wall

        self.was_drifting[env_index] = currently_drifting

        # Mushroom Timing Reward
        old_mushroom_count = old_state.get("mushrooms", 0)
        new_mushroom_count = new_state["mushrooms"]

        if new_mushroom_count < old_mushroom_count:
            # A mushroom was used
            self.episode_mushrooms_used[env_index] += 1

            if speed > 50 and abs(new_state["track_center_dist"]) < 0.5:
                reward += 3.0  # Good timing: fast, on a straight
            elif abs(new_state.get("angular_velocity", 0)) > 0.15:
                reward -= 1.0  # Used mid-turn
                self.episode_mushrooms_wasted[env_index] += 1

            if new_state["wall_hit"] > 0.5:
                reward -= 3.0  # Wasted mushroom (hit wall immediately)
                self.episode_mushrooms_wasted[env_index] += 1
    ...
    def reset_at(self, index: int) -> Tuple[np.ndarray, Dict]:
        ...
        try:
            ...
            obs, state_dict = self._parse_and_preprocess(state_bytes, env_index=index, old_wall_values=None)
            ...
            self.drift_counters[index] = 0.0
            self.last_angles[index] = 0.0
            self.was_drifting[index] = False
            self.episode_drift_boosts[index] = 0
            self.episode_mushrooms_used[index] = 0
            self.episode_mushrooms_wasted[index] = 0
    ...
    def step(self, s_actions: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, Dict]:
        ...
        for i in range(self.num_envs):
            ...
            new_frame, new_state = self._parse_and_preprocess(state_bytes, env_index=i, old_wall_values=old_walls)
            ...
            reward, terminated, is_finished_race = self._calculate_reward(i, old_state, new_state)
            ...
            if terminated or truncated:
                ...
                info["final_info"] = {
                    "episode": {
                        ...
                        "drift_boosts": int(self.episode_drift_boosts[i]),
                        "mushrooms_used": int(self.episode_mushrooms_used[i]),
                        "mushrooms_wasted": int(self.episode_mushrooms_wasted[i]),
                    }
                }
                if is_finished_race:
